...
armstrong <- armstrong  %>% filter(Latitude != 'NA') # drop one record that does have any coordinates
...
armstrong_latLon_9 <- as.data.frame(utm2lonlat(easting = armstrong_utm_9$Easting, northing = armstrong_utm_9$Northing, zone = 9, hemisphere = 'N')) %>% rename(newLongitude = longitude, newLatitude = latitude)
