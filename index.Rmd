...
5| `malus_sdm.R` | Split training/testing data, Max models, habitat predictions
...
```{r, eval = TRUE, echo=FALSE, out.width= '600px'}
knitr::include_graphics(rep("C:/Users/terre/Documents/UBC/Botanical Garden/Malus Project/colinearity/cor_wclim_colinearity.jpeg"))
```
...
```{r, eval = TRUE, echo=FALSE, out.width= '600px'}
knitr::include_graphics(rep("C:/Users/terre/Documents/UBC/Botanical Garden/Malus Project/colinearity/cor_dendo.jpeg"))
```
...
```{r, eval = TRUE, echo=FALSE, out.width= '600px'}
knitr::include_graphics(rep("C:/Users/terre/Documents/UBC/Botanical Garden/Malus Project/checkboard_aggregation/cor_cb_pr.jpeg"))
```
...
```{r, eval = TRUE, echo=FALSE, out.width= '600px'}
knitr::include_graphics(rep("C:/Users/terre/Documents/UBC/Botanical Garden/Malus Project/checkboard_aggregation/cor_cb_bg_3,3.jpeg"))
```

**6,6** aggregation

```{r, eval = TRUE, echo=FALSE, out.width= '600px'}
knitr::include_graphics(rep("C:/Users/terre/Documents/UBC/Botanical Garden/Malus Project/checkboard_aggregation/cor_cb_bg_6,6.jpeg"))
```

**9,9** aggregation, which I chose to use!

```{r, eval = TRUE, echo=FALSE, out.width= '600px'}
knitr::include_graphics(rep("C:/Users/terre/Documents/UBC/Botanical Garden/Malus Project/checkboard_aggregation/cor_cb_bg_9,9.jpeg"))
```

**12,12** aggregation

```{r, eval = TRUE, echo=FALSE, out.width= '600px'}
knitr::include_graphics(rep("C:/Users/terre/Documents/UBC/Botanical Garden/Malus Project/checkboard_aggregation/cor_cb_bg_12,12.jpeg"))
```
...
* Next the the `envs` argument is `SpatRaster` of bioclimatic predictor data cropped to the **background extent** (or environment). We can reuse the cropped raster we produced in the `malus_bg.R` script. 
...
The next step after running several  model combinations (96 to be percise), is to select what 'best' model (with particular tuning arguments). A common approach is to use Akaike information criterion (AIC), specifically delta AICc, which is a corrected AIC value, expressed as the relative difference ib values. Delta AICc values less than 2, are considered to be equally good models. In my case, there was only a single model less than 2, the best performing model.
...
```{r, eval = TRUE, echo=FALSE, out.width= '600px'}
knitr::include_graphics(rep("C:/Users/terre/Documents/UBC/Botanical Garden/Malus Project/maps/habitat suitability/Predicted Suitability/coronaria_historical_raw.jpeg"))
```
...
First, is how confident can we be of these predictions? Using metrics like Area Under the Curve (AUC) and Continous Boyce Index can help us, however they have their own flaws, and so its important to **think critically** about the results of your predictions. 

Second, is it best to leave these suitability scores as a continuous gradient of habitat suitability? Myself (and many others) find maps with colour gradients illegible, or less informative. Some take the approach of creating a binary threshold, where habitat suitability is boolean. There are several methods to determine this binary threshold, which can be done using the `predicts` package. A flaw of this approach is it less tangible what these thresholds mean in lay terms.
...
However, I find it more practical to bin suitability scores into several **categories**. Lets say three categories: "High suitability", "Moderate suitability" and "Low suitability". These categories should be **determined using qauntiles**, where suitability scores in the lowest quantiles can be considered to be lowly or moderately suitable, and higher scores considered to be of high suitability. In this case I use the 1st, 10th, and 50th percentile to classify these categories. This means that in the low habitat category we are only committing 1% of predicted values, compared to the 50th, were we are omitting 50% of predicted values.
...
```{r, eval = TRUE, echo=FALSE, out.width= '600px'}
knitr::include_graphics(rep("C:/Users/terre/Documents/UBC/Botanical Garden/Malus Project/maps/habitat suitability/Predicted Suitability/coronaria_historical_bin.jpeg"))
```

Now we can move on to make predictions for the future climate scenarios. We will still use the **same threshold** values from the historical prediction to keep interpretation consistent. As always, make sure to save them.
...
For the purposes of this tutorial I am only going to show results of one of the future climate predictions - SSP5-8.5 2020-2040 (the current tracking projections). I am using the same plot code as above for the historical prediction.
